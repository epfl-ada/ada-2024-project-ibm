...
def load_distance_matrix(filepath="dataset\shortest-path-distance-matrix.txt"):
    ...
    for line in lines:
        ...
        ):  # Skip comments and empty lines ->strip() method removes all leading and trailing whitespace characters from a string; if empty -> ''
            continue
        row = []
        for char in line.strip():
            if char == "_":  # Replace unreachable distances with infinity
                row.append(np.nan)
            else:  # Convert digit characters to integers
                row.append(int(char))
        matrix.append(row)
...
def load_unfinished_paths(file_path="./dataset/paths_unfinished.tsv"):
    ...
    # Eliminate target articles which arent in identified in categories (cannot be reached)
    # NO -> cannot reach wrong
    """
    df["target"] = df["target"].replace("Long_peper", "Long_pepper")
    df["target"] = df["target"].replace("Podcast", "Podcasting")
    df["target"] = df["target"].replace("Macedonia", "Republic_of_Macedonia")
    df["target"] = df["target"].replace("Bogota", "Bogot%C3%A1")
    df["target"] = df["target"].replace("Usa", "United_States")
    df["target"] = df["target"].replace("Kashmir", "Kashmir_region")
    """
    target_unfinished = df["target"].unique()
    articles = df_categories["article"].unique()

    # Convert to sets
    target_unfinished_set = set(target_unfinished)
    articles_set = set(articles)

    # Items in `target_unfinished` but not in `articles`
    unfinished_not_in_articles = target_unfinished_set - articles_set
    df = df[~df["target"].isin(unfinished_not_in_articles)]

    # Add column for path length
    df["path_length"] = df["path"].apply(lambda x: len(x.split(";")))  # Nor remove any

    # Filter to only include path length greater than 2
    df = df[df["path_length"] > 2]  # Denotes no intention of finding sol.

    # Filter by 'restart'
    df = df[df["type"] == "restart"]

    # For ratio: time/path -> min :   2.285714 (plausible no filter)
    df["avg_time_click"] = df["durationInSec"] / (df["path_length"])
    df = df[df["avg_time_click"] <= 180]
...
def load_finished_paths(file_path="./dataset/paths_finished.tsv"):
    ...
    # Create path length columns
    df["path_length"] = df["path"].apply(lambda x: len(x.split(";")))  # Not remove any

    # Eliminate those with same source and target
    df = df[df["path_length"] > 1]

    # Filter to only include path length greater than 2 (source, target)
    df = df[df["path_length"] > 2]  # Trivial (too easy)

    # For ratio: time/path -> min :  1.750000 (plausible no filter)
    df["avg_time_click"] = df["durationInSec"] / (
        df["path_length"] - 1
    )  # compensate for including  target
    df = df[df["avg_time_click"] <= 180]
